...
async def nats_todo(msg):
  """Publishes msg to nats"""
  await nc.connect(servers=[os.getenv('NATS_URL')])

  print('sending msg')
  await nc.publish("updates", str.encode(msg))
  print('msg sent')


# "/" endpoint
def home(request):
    """Index endpoint for todo app"""
    return JsonResponse({'hello': 'todos'})


# "/image" endpoint
def image(request):
    """Image endpoint for todo app"""
    get_image()
...
def todos(request):
    ...
    if request.method == 'PUT':
        ...
        try:
            ...
            asyncio.run(nats_todo(f'removed todo: {todo_id}'))
